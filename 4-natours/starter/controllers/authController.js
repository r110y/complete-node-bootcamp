...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    ...
    // secure: true, // ONLY send with HTTPS
    httpOnly: true,
  };
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  // 2) Validate token signature with jwt (verification)
  const decoded = await promisify(jwt.verify)(
    token,
    process.env.JWT_SECRET,
  );

  // 3) If verification successful, check if user exists
...
exports.restrictTo =
  (...roles) =>
  (req, res, next) => {
    // roles array of strings ['admin', 'lead-guide'];
...
exports.resetPassword = catchAsync(
  async (req, res, next) => {
    ...
    // 2) If token not expired && user, set new password
    if (!user) {
      return next(
        new AppError(
    ...
    // Don't need to turn off validators, i.e. want to check if pw = pw confirm.
    await user.save();
